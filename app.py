...
def get_financial_recommendation(user_id):
    ...
    if df.empty:
        return "Add some transactions to get personalized financial advice."

    total_income = df[df['type'] == 'income']['amount'].sum()
    total_expenses = df[df['type'] == 'expense']['amount'].sum()

    if total_income == 0:
        return "Add income data to receive tailored saving and investment advice."

    spending_ratio = total_expenses / total_income

    if spending_ratio > 0.8:
        return ("\U0001F6A8 You're spending more than 80% of your income. Consider cutting down discretionary expenses. "
                "Try using the 50/30/20 rule: 50% for needs, 30% for wants, 20% for savings.")
    elif spending_ratio > 0.5:
        return ("⚠️ You’re spending moderately. Great job! To optimize, you could explore SIPs, "
                "mutual funds or recurring deposits for long-term gains.")
    else:
        return ("✅ Excellent! You’re saving over 50% of your income. Consider investing in low-risk "
